...
class EMA(Module):
    ...
    def update_moving_average(self, ma_model, current_model, current_decay=None):
        ...
        for (name, current_params), (_, ma_params) in zip(
            ...
            if (
                name in self.param_or_buffer_names_no_ema
            ...
            if not self.use_foreach:
                self.inplace_lerp(
                    ma_params.data, current_params.data, 1 - current_decay
                )
            else:
        ...
        for (name, current_buffer), (_, ma_buffer) in zip(
            ...
            if not self.use_foreach:
                self.inplace_lerp(
                    ma_buffer.data, current_buffer.data, 1 - current_decay
                )
            else:
        ...
        if len(tensors_to_copy) > 0:
            if self.use_foreach:
                torch._foreach_lerp_(
                    tensors_to_lerp, tensors_to_copy, 1 - current_decay
                )
