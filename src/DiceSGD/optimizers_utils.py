    ...
    def _create_noisy_clipped_dice_gradient(self):
        ...
        for name, param in list(
            ...
            if not hasattr(param, "summed_clipped_grad"):
                ...
                print(
                    name,
                    " this parameter has summed clipped grad but does not have grad",
                )
        ...
        for name, param in self.named_params:
            if param.requires_grad:
                if hasattr(param, "error"):
                    ...
                    # param.error = None
                    print(f"Parameter {name} does not have 'error' attribute.")
                    first_minibatch = True
                    error_norms.append(torch.tensor(0.0))
        ...
        for name, param in self.named_params:
            ...
            if first_minibatch:
                ...
                param.grad += param.error * torch.clamp_max(
                    self.max_grad_norm / error_norm * self.error_max_grad_norm, 1
                )
                param.error = (
                    param.error
                    * torch.clamp_max(
                        self.max_grad_norm / error_norm * self.error_max_grad_norm,
                        (
                            1
                            - torch.clamp_max(
                                self.max_grad_norm
                                / error_norm
                                * self.error_max_grad_norm,
                                1,
                            )
                        ),
                    )
            ...
            if self.noise_multiplier > 0 and self.max_grad_norm > 0:
                noise = torch.normal(
                    ...
                    * self.max_grad_norm
                    * math.sqrt(1 + 2 * self.error_max_grad_norm),
                    size=param.size(),
                    device=param.device,
                    dtype=param.dtype,
                )
