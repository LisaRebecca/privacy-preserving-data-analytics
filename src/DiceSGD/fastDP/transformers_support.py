...
def forward_swapper(module):
    ...
    if isinstance(
        module,
        (transformers.OpenAIGPTLMHeadModel, transformers.OpenAIGPTDoubleHeadsModel),
    ):
        swap_openai_gpt_model_forward(module.transformer)
    if isinstance(
        module, (transformers.GPT2LMHeadModel, transformers.GPT2DoubleHeadsModel)
    ):
...
def swap_gpt2_model_forward(
    model: Union[transformers.GPT2Model, transformers.GPT2DoubleHeadsModel]
):
...
def swap_t5_model_forward(model: nn.Module):
    ...
    def compute_bias(self, query_length, key_length, batch_size, device=None):
        ...
        relative_position_bucket = (
            relative_position_bucket.reshape(-1).unsqueeze(0).repeat(batch_size, 1)
        )
        values = self.relative_attention_bias(
            relative_position_bucket
        )  # shape (batch_size, q_len x k_len, num_heads)
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                position_bias = torch.zeros(
                    (batch_size, self.n_heads, real_seq_length, key_length),
...
def freeze_isolated_params_for_vit(model):
    ...
    for module in model.modules():
        ...
        if isinstance(
            module, transformers.models.beit.modeling_beit.BeitRelativePositionBias
        ):
