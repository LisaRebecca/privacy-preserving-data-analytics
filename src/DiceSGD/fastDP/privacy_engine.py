...
class PrivacyEngine(object):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        if clipping_mode not in ["ghost", "MixGhostClip", "MixOpt"]:
            raise ValueError(
                f"Unknown clipping mode {clipping_mode}. Expected one of 'ghost','MixGhostClip','MixOpt'."
            )
        if accounting_mode not in ("rdp", "all", "glw"):
            raise ValueError(
                f"Unknown accounting mode: {accounting_mode}. Expected one of 'rdp', 'all','glw'."
            )
        if epochs is None:
            if num_steps is not None:
                ...
                raise ValueError(
                    f"Number of training epochs and training steps are not defined."
                )
        ...
        for name, param in module.named_parameters():
            ...
            if origin_params != None:
                param.requires_grad = param.initially_requires_grad and any(
                    [i in name for i in origin_params]
                )  # only requires grad if it is origin and initially requires grad
        ...
        def _supported_and_trainable(layer):
            if type(layer) in _supported_layers_norm_sample_AND_clipping and (
                (
                ...
                or (
                    hasattr(layer, "bias")
                    and hasattr(layer.bias, "initially_requires_grad")
                    and layer.bias.initially_requires_grad
                )
            ):
                return True
            return False
        ...
        self.n_layers = len(
            self.named_layers
        )  # sum(1 for layer in module.modules() if autograd_grad_sample.requires_grad(layer) and hasattr(layer,'weight'))

        self.n_components = 0
        for name, layer in self.named_layers:
            self.n_components += sum(
                [1 for p in layer.parameters() if p.initially_requires_grad]
            )
        print(
            "Number of trainable components: ",
            self.n_components,
        ...
        # -----
        print(
            ">>>>>>>>>>>>>>>>> Applying ", clipping_fn, " per-sample gradient clipping."
        )
        ...
        if clipping_style == "layer-wise":
            ...
            self.max_grad_norm_layerwise = self.max_grad_norm / math.sqrt(
                self.n_components
            )
        ...
        for name, param in module.named_parameters():
            ...
            if torch_seed_is_fixed == True:
                ...
                param.noise = (
                    self.noise_multiplier * self.max_grad_norm / math.sqrt(num_GPUs)
                )
        ...
        print(
            ">>>>>>>>>>>>>>>>> Block heads for per-sample gradient clipping are defined as:",
            self.block_heads,
        )

        transformers_support.forward_swapper(
            module=module
        ...
        autograd_grad_sample.add_hooks(
            ...
            bias_only=self.bias_only,
            clipping_style=self.clipping_style,
        ...
        self.module.get_training_stats = types.MethodType(
            get_training_stats, self.module
        )
                        ...
                        if param.grad is not None:
                            if hasattr(param, "summed_clipped_grad"):
                                param.grad = torch.nan_to_num(
                                    param.summed_clipped_grad
                                ).contiguous() + torch.normal(
                                    mean=0,
                                    std=param.noise,
                                    size=param.size(),
                                    device=param.device,
                                ...
                                param.grad = (
                                    param.grad / param.batch_size * self.loss_scale
                                )  # it works
                            else:
                                param.grad.zero_()
    ...
    def _create_noisy_clipped_gradient(self):
        ...
        for name, param in list(
            self.named_params
        ):  # https://thispointer.com/python-remove-elements-from-a-list-while-iterating/#1
            if not hasattr(param, "summed_clipped_grad"):
                unsupported_param_name.append(name)
                self.named_params.remove(
                    (name, param)
                )  # very helpful for models that are not 100% supported, e.g. in timm
        if unsupported_param_name != []:
            print(
                unsupported_param_name,
                "are not supported by privacy engine; these parameters are not requiring gradient nor updated.",
            )
        ...
        if self.record_snr and len(noises) > 0:
            self.signal, self.noise = tuple(
                torch.stack(lst).norm(2).item() for lst in (signals, noises)
            )
            self.noise_limit = (
                math.sqrt(self.num_params) * self.noise_multiplier * self.max_grad_norm
            )
