...
from .supported_layers_grad_samplers import (
    _supported_layers_norm_sample_AND_clipping,
    _create_or_extend_summed_clipped_grad,
)
import numpy as np
...
def requires_grad(module: nn.Module) -> bool:
    ...
    """
    return any(
        p.initially_requires_grad
        for p in module.parameters()
        if hasattr(p, "initially_requires_grad")
    )
...
def add_hooks(
    ...
    for name, layer in model.named_modules():
        if type(layer) in _supported_layers_norm_sample_AND_clipping and requires_grad(
            layer
        ):
            if (
                hasattr(layer.weight, "initially_requires_grad")
...
def _prepare_sample_grad_or_norm(
    ...
    if loss_reduction == "mean":
        backprops = (
            backprops * backprops.shape[0]
        )  # .backprops should save dL_i/ds, not 1/B*dL_i/ds, the mean reduction is taken care of in privacy engine .step()
    compute_layer_grad_sample, _ = _supported_layers_norm_sample_AND_clipping.get(
        type(layer)
    )
...
def _per_block_clip_grad(
    ...
    if clipping_fn == "automatic":
        ...
        C = torch.clamp_max(
            max_grad_norm_layerwise / (norm_sample + numerical_stability_constant), 1.0
        )
    ...
    for name, layer in named_layers:
        if (
            ...
            and layer.weight.initially_requires_grad
            and hasattr(layer, "activations")
            and hasattr(layer.weight, "norm_sample")
        ):
            # --- weight, compute clipped gradient
            _, compute_layer_grad = _supported_layers_norm_sample_AND_clipping.get(
                type(layer)
            )
            grad_weight = compute_layer_grad(
                layer,
                layer.activations,
                torch.einsum("b...,b->b...", layer.backprops, C),
                C,
            )
        ...
        if (
            ...
            and hasattr(layer.bias, "grad_sample")
            and hasattr(layer.bias, "norm_sample")
        ):
